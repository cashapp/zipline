...
import app.cash.plywood.read
import app.cash.plywood.sink
import app.cash.plywood.source
...
class FakeKotlinStdlib {
  ...
  private inner class Service(
    ...
    // This is how System.out works.
    fun fd_write(fd: Int, iovec: Int, len: Int, written: Int): Int {
      val iovecSource = memory.source(iovec.toLong()).buffer()
      val resultSink: BufferedSink = memory.sink(written.toLong()).buffer()

      for (i in 0 until len) {
        val dataPointer = iovecSource.readIntLe()
        val count = iovecSource.readIntLe()
        val content = memory.read(dataPointer.toLong()) {
          readByteArray(count.toLong())
        }
        print(content.decodeToString())
        resultSink.writeIntLe(count)
      }

      return 0
    }

    fun asImports(spec: WasmModule.Spec): Array<WasmExternal> {
      return arrayOf<WasmExternal>(
        object : WasmFunction {
          override val spec = spec.imports.first { it.name == "fd_write" } as WasmFunction.Spec
        ...
        object : WasmFunction {
          override val spec =
            spec.imports.single { it.name == "clock_time_get" } as WasmFunction.Spec
